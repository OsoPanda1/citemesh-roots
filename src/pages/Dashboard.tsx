...
import { Activity, Server, Wifi, Cpu, HardDrive, Zap, AlertTriangle, Eye } from "lucide-react";
...
const domainHealth = [
  { name: "ID-NVIDA", value: 95 }, { name: "UTAMV", value: 88 },
  { name: "Metaverso", value: 82 }, { name: "Economía", value: 91 }, { name: "Seguridad", value: 98 },
];
const COLORS = ["hsl(38, 90%, 55%)", "hsl(185, 70%, 45%)", "hsl(280, 60%, 55%)", "hsl(120, 50%, 45%)", "hsl(0, 70%, 55%)"];
const timelineData = [
  { hora: "00:00", carga: 12 }, { hora: "04:00", carga: 8 }, { hora: "08:00", carga: 45 },
  { hora: "12:00", carga: 78 }, { hora: "16:00", carga: 65 }, { hora: "20:00", carga: 42 }, { hora: "23:59", carga: 18 },
];
const radarData = [
  { metric: "Disponibilidad", value: 96 }, { metric: "Rendimiento", value: 88 }, { metric: "Seguridad", value: 98 },
  { metric: "Latencia", value: 75 }, { metric: "Resiliencia", value: 92 }, { metric: "Escalabilidad", value: 84 },
];
const statCards = [
...
const StatusDot = ({ status }: { status: "ok" | "warn" | "error" }) => (
  <span className={`w-2 h-2 rounded-full inline-block ${status === "ok" ? "bg-green-500 animate-pulse" : status === "warn" ? "bg-yellow-500 animate-pulse" : "bg-destructive animate-pulse"}`} />
);
...
const Dashboard = () => (
  <WikiPage title="Dashboard de Monitoreo" subtitle="Estado en tiempo real de nodos federados y dominios TAMV">
    <Section title="Visibilidad por nivel de membresía">
      <div className="rounded-lg border border-border/50 bg-muted/20 p-4 mb-2">
        <div className="flex items-start gap-2 mb-3">
          ...
          <p className="text-sm text-muted-foreground">La información visible varía según tu nivel de membresía:</p>
        </div>
    ...
    <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
      {statCards.map((s, i) => (
        <motion.div key={s.label} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }} className="rounded-lg border border-border/50 bg-card/60 p-3">
          <div className="flex items-center justify-between mb-2"><s.icon className="h-4 w-4 text-muted-foreground" /><StatusDot status={s.status} /></div>
          ...
          <ResponsiveContainer width="100%" height={260}>
            <PieChart>
              <Pie data={domainHealth} dataKey="value" nameKey="name" cx="50%" cy="50%" outerRadius={90} label={({ name, value }) => `${name}: ${value}%`}>
                {domainHealth.map((_, i) => (<Cell key={i} fill={COLORS[i % COLORS.length]} />))}
              </Pie>
              <Tooltip contentStyle={{ background: "hsl(220, 18%, 10%)", border: "1px solid hsl(220, 15%, 18%)", borderRadius: 8, color: "hsl(45, 10%, 90%)" }} />
            </PieChart>
          </ResponsiveContainer>
          ...
          <tbody>
            {nodeStatus.map((n) => (
              <tr key={n.name} className="border-t border-border/30 hover:bg-muted/10 transition-colors">
                ...
                <td className="px-4 py-2.5 text-center"><StatusDot status={n.inactivos === 0 ? "ok" : "warn"} /></td>
              </tr>
            ))}
          </tbody>
        </table>
